...
class MidiReader {
  ...
  MidiEvent readEvent(ByteReader p) {
    ...
    if ((eventTypeByte & 0xf0) == 0xf0) {
      ...
      if (eventTypeByte == 0xff) {
        return _parseMetaEvent(p, deltaTime);
      ...
      return _parseChannelEvent(p, deltaTime, eventTypeByte);
    }
  }

  /// Parses provided [data] and returns a list of [MidiEvent]
  MidiTrack parseTrack(List<int> data) {
  ...
  void _linkNoteEventPairs(List<List<MidiEvent>> tracks) {
    for (var track in tracks) {
      // dim 1: track channel
      // dim 2: note number
      final noteOns = List<List<Queue<NoteOnEvent>>>.filled(
          16, List.filled(128, Queue<NoteOnEvent>()));
      for (var event in track) {
        if (event is NoteOnEvent) {
          noteOns[event.channel][event.noteNumber].add(event);
        } else if (event is NoteOffEvent) {
          final q = noteOns[event.channel][event.noteNumber];
          if (q.isEmpty) {
            throw "NoteOff event encountered before NoteOn event";
          } else {
            final t = noteOns[event.channel][event.noteNumber].removeFirst();
            t.linkNoteOff(event);
          }
        }
      }
  ...
  MidiEvent _parseChannelEvent(ByteReader p, int deltaTime, int eventTypeByte) {
    // channel event
    int param1;
    bool running = false;
  ...
  MidiEvent _parseMetaEvent(ByteReader p, int deltaTime) {
    // meta event
